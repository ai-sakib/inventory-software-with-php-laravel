				...
				<tr id="tr-{{$value->id}}">
                    ...
                    <td id="clear-each-{{ $value->id }}">
                      @if($value->total_price - $value->paid > 0)
                        <a onclick="setPayment('{{ $value->id }}','{{ $value->total_price }}','{{ $value->total_price - $value->paid }}','{{ $value->customer_id }}','{{ $value->customer_id }}')" class="btn btn-xs bg-info"><i class="fas fa-money-bill-alt "></i>&nbsp;Pay</a>&nbsp;&nbsp;<a onclick="clearPaymentEach('{{ $value->id }}','{{ $value->customer_id }}')" class="btn btn-xs bg-success"><i class="fa fa-window-close "></i>&nbsp;Clear</a>
                      @else
...
<script type="text/javascript">
	function setPayment(id,total,due,customer_id) {
    $.confirm({
      title: 'Payment',
      content:
          ...
          formSubmit: {
              ...
              action: function () {
                  ...
                  if(payment > 0){
                    if(payment <= due){
                      $.ajax({
                        url: "{{url('customers')}}/"+id+'&'+payment+"/setPayment",
                        type: 'GET',
                        data: {},
                      })
  ...
  function clearPaymentEach(id,customer_id) {
    $.confirm({
      title: '',
      ...
      buttons: {
          confirm: {
              ...
              action: function(){
                $.ajax({
                url: "{{url('customers')}}/"+id+'/clearPaymentEach',
                type: 'GET',
                dataType: 'json',
