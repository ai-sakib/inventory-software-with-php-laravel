          ...
          <form action="{{ route('sub-menu.store') }}" method="post">
            {{ csrf_field() }}
                      ...
                      <select class="form-control" name="main_menu_id">
                        @if(isset($mainMenus[0]->id))
                          @foreach($mainMenus as $mainMenu)
                            <option value="{{ $mainMenu->id }}" @if(old('main_menu_id') == $mainMenu->id) selected @endif>{{ $mainMenu->name }}</option>
                          @endforeach
                        @endif
                      </select>
                    </div>
                ...
                <div class="form-group">
                  ...
                  <input type="text" id="name" class="form-control" name="name" value="{{old('name')}}" placeholder="Sub Menu Name" required>
                </div>
                <div class="form-group">
                  <label for="link">Link</label><span style="color:red"> *</span>
                  <input type="text" id="link" class="form-control" name="link" value="{{old('link')}}" placeholder="Link" required>
                </div>
                <div class="form-group">
